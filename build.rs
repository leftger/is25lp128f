#![doc = "Build script that generates the IS25LP128F register API from the YAML manifest."]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-changed=src/is25lp128f.yaml");

    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR must be set"));
    let manifest_path = manifest_dir.join("src/is25lp128f.yaml");
    let manifest = fs::read_to_string(&manifest_path)?;

    let generated = device_driver_generation::transform_yaml(&manifest, "Is25lp128fDevice");
    ...
    let dest = out_dir.join("is25lp128f_device.rs");
