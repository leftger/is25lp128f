...
mod embedded_example {
    ...
    use embassy_stm32::{
        ...
        rcc::{
            AHB5Prescaler, AHBPrescaler, APBPrescaler, PllDiv, PllMul, PllPreDiv, PllSource,
            Sysclk, VoltageScale,
        },
    ...
    impl<'a> Spi for EmbassySpiFlash<'a> {
        ...
        async fn transfer_in_place(&mut self, buf: &mut [u8]) -> Result<(), is25lp128f::Error> {
            ...
            let r = self
                ...
                .await
                .map_err(|_| is25lp128f::Error::SpiConfigError);
            self.cs_high();
            r
        }
    ...
    async fn main(_spawner: Spawner) {
        ...
        if let Ok(jedec) = flash.read_jedec_id().await {
            info!(
                "JEDEC ID: mfr=0x{:02x} type=0x{:02x} cap=0x{:02x}",
                jedec.mfr_code, jedec.memory_type, jedec.capacity
            );
        }
        ...
        if let Err(e) = NorFlash::erase(&mut flash, OFFSET, sector_end).await {
            ...
            if NorFlash::write(&mut flash, OFFSET, &write_data)
                .await
                .is_ok()
            {
                info!("Wrote 256 bytes at 0");
                let mut read_back = [0u8; 256];
                if ReadNorFlash::read(&mut flash, OFFSET, &mut read_back)
                    .await
                    .is_ok()
                {
